...
//         // 1. [SQL Syntax Error] 따옴표 개수 확인 (홀수면 에러)
//         // 시뮬레이션: 쿼리가 닫히지 않아 발생하는 에러
...
//         // 2. [WAF 업그레이드] 숫자형 논리(Tautology) 패턴 정밀 차단
//         // 설명: 정규식 /\d+=\d+/ 은 "숫자=숫자" 형태를 잡아냅니다.
//         // 예: 1=1, 2=2, 99=99 등이 모두 걸러집니다.
//         if (/[\d]+=[']?[\d]+/.test(lowerUser)) { 
//             setMessage("🚫 [Security Warning] Numeric Logic Injection pattern (e.g., 1=1, 2=2) detected. Blocked by WAF.");
//             return;
//         }

//         // 3. 공격 성공 조건 확인 (More Advanced)
//         // 조건 1: 주석(-- 또는 #)은 필수
//         const hasComment = username.includes('--') || username.includes('#');
        
//         // 조건 2: "문자열"이 같다('a'='a')는 논리가 포함되어야 함
//         // 숫자=숫자는 위에서 막혔으므로, 이제는 '문자'='문자' 형태만 통과 가능
//         // 정규식 설명: '어떤문자'='어떤문자' 패턴이 있는지 확인
//         const isStringHack = /'[^']+'='[^']+'/.test(lowerUser);

//         if (hasComment && isStringHack) {
//             navigate('/admin-secret'); // 성공 시 이동
//         } else if (username === 'admin' && password === 'real_complex_password') {
//             navigate('/admin-secret'); // 정석 로그인
//         } else {
//             // 4. 일반 실패
